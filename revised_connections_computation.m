...
[dphii_l1, phii_l1, x_l1] = get_STM(t_dc(end),X_dc(1,:)',4,1);  % Taking out STM for periodic orbit from TBP but now calculating it for FBP
...
N2 = 35; % Total number of discrete points on invariant circle (KEEP IT ODD)  %35 (l1)  %29 (l2)
...
psued_arc = 1e-18;   % -1e-4 (l1 3e-3)   1e-12 (l2 2e-2) 
...
newton_iter = 13;  % (l1 35| l2 22)  (NEW: 2_l1.mat - 9)
...
for h=1:newton_iter
           ...
           for i=1:N
               ...
               [J_lmda, J, G] = BVP(X_, tau_interval(1), 1/N, m, N2, [X_interval(end-1), X_interval(end), ((a_3/a_2)-mu1), n_3, mu1, mu2, l_pos, w_coll0], D_rho);
           ...
           fff = F_coll(X_prev,[0,0],X_interval(end),[(a_3/a_2)-mu1,0],[n_3, mu1, mu2, 4, l_pos]);  % For phase condition
           Xdiff = (X_init-X_prev);
           ...
           plot_collected(X_interval(1:4*N2),'position',[1 0 0]);    % X0
           plot_collected(X_interval((m+1)*4*N*N2 + 1: (m+1)*4*N*N2 + 4*N2),'position',[0 0 1]);   % Xends
...
% load('good plots\plots_for_paper\1_l1.mat');
% load('good plots\plots_for_paper\1_l1data.mat');
% Jtot_l1 = J_total;
% Gtot_l1 = G_total;
% 
% load('good plots/plots_for_paper/1_l2.mat');
% load('good plots/plots_for_paper/1_l2data.mat');
% Jtot_l2 = J_total;
% Gtot_l2 = G_total;
% 
...
% plot_collected(Xl2_dc(1:4*N2_l2),'position',[1 0 0])
% R_europa = 1560000/DU;syms T;fplot((1-mu1)+R_europa*sin(T),R_europa*cos(T));
% 
% % Stable/unstable manifold calculation
% % L2 stable and L1 unstable manifold
% epsilon = 1e-6;
% tspan_st = 0:-0.01:-0.5*pi/abs(n_3-1);  % for L2
% tspan_unst = 0:0.01:0.5*pi/abs(n_3-1);  % for L1
...
% 
% %kk = 25; % Just a switch to other points in invariant circle (limit 29 for l2, 35 for l1)
% for kk=0:N2_l2-1
%     [T_st, M_st] = ode45(@(t,x) PCC4BP_eqn(t,x,mu1,mu2,[(a_3/a_2)-mu1,0], 2*pi, 3), tspan_st, Xst0(1+4*kk:4+4*kk));
%     %[T_unst, M_unst] = ode45(@(t,x) PCC4BP_eqn(t,x,mu1,mu2,[(a_3/a_2)-mu1,0], 2*pi, 3), tspan_unst, Xunst0(1+4*kk:4+4*kk));
%     hold on; plot(M_st(:,1),M_st(:,2),'-b'); 
%     %hold on; plot(M_unst(:,1),M_unst(:,2),'-g')
%     pause(0.00001)
% end
...
function [J_lmda, J, G] = BVP(x,tau,tau_diff,m,N2,extras,D_rho)
...
for j=1:m  % iterating over collocation points btw a single tau interval
    ...
    for k=1:m+1
        ...
        if (k-1)~=j
            ...
            JJ_block = [];
            for i=1:N2
                J_block = PCC4BP_J(x_dyn(1+4*(i-1)), x_dyn(2+4*(i-1)), sign(n_3-1)*2*pi*tau_j, mu1, mu2, extras(3));
