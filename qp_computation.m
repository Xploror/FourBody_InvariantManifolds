...
N2 = 31; % Total number of discrete points on invariant circle (KEEP IT ODD)
...
DFT = zeros(length(X_coll0(1,:))*N2,length(X_coll0(1,:))*N2);
Q = zeros(length(X_coll0(1,:))*N2,length(X_coll0(1,:))*N2);
w = exp(-1i*2*pi/N2);
for i=1:N2
...
for j=0:0
    for h=1:newton_iter
               J_total = [[kron(zeros(N2,N2),zeros(4,4)) zeros(4*N2,2)]; zeros(2,4*N2+2)];  % (Vector + quasi-periodic + phase + pseudo-arclength)
               G_total = [];
               % Adding collocation + continuity condition
               for i=1:N2
                   tau_interval = [(i-1)/N2 i/N2];
                   ...
                   J_total(1+4*(m+1)*(i-1):(4*(m+1))+4*(m+1)*(i-1), 1+4*(m+1)*(i-1):(4*(m+2))+4*(m+1)*(i-1)) = J;
                   G_total = [G_total; G];
               end
               % Adding QuasiPeriodic Condition
               Rot = real(KKK(end-3:end, end-3:end));
               G_total = [G_total; X_interval(1:4)-Rot*X_interval(end-3:end)];
               J_total(end-4:end-1,end-3:end) = -Rot; J_total(end-4:end-1,1:4) = eye(4); 
               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     col = rand(1,3);
%     plot_collected(X_interval, 'position', col);
%     pause(0.01)
...
for j=1:m  % iterating over collocation points btw a single tau interval
    ...
    for k=1:m+1
        ...
        if (k-1)~=j
            ...
            J(1+4*(j-1):4+4*(j-1), 1+4*(k-1):4+4*(k-1)) = tau_diff*diff_lagrange*eye(4);
        else
            % Jacobian collocation section (includes diagonals) % (2 to m+1)
            J_block = PCC4BP_J(x(1+4*j), x(2+4*j), 2*pi*tau_j, mu1, mu2, extras(3));
            J(1+4*(j-1):4+4*(j-1), 1+4*j:4+4*j) = tau_diff*diff_lagrange*eye(4) - (J_block(1:4, 1:4) - lmda*S_pert*vf_pert(x(1+4*j:4+4*j), 2*pi*tau_j, l1_pos_io));
        end
    end
    g = tau_diff*S - f;  % 4x1
    G = [G; g];
